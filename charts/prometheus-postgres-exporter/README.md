...
This chart bootstraps a prometheus [postgres exporter](https://github.com/nholuongut/postgres_exporter) deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
```console
helm install [RELEASE_NAME] nholuongut/prometheus-postgres-exporter
```
...
```console
helm show values nholuongut/prometheus-postgres-exporter
```
